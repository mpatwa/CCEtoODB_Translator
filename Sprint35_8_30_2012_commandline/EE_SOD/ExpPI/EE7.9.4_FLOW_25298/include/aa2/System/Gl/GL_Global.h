...
public:
	...
	static int IsPolyInList(GeomHdr *list, GeomHdr *poly);
	...
	static int	IsChar (char c, char *str);	// return k > 0 in case str[k-1] == c
	static char *GetCtrlStr(char *inpStr, char *ctrlStr, char **cstr);	// return clean str, and cstr - active ctrl
	...
	static int	IsCrossLists(GeomHdr *gh1, GeomHdr *gh2);
	static int	CheckList1(GeomHdr *gh1, int is_prev = 1);
	static int CheckList(GeomHdr *first, int &num_sdb, int &num_scan, int &num_hide, stScan **first_scan, char *str);
	static int CheckAf(int layer, int id, char *str);
...
/////////////////////////// pnts //////////////////
extern char *sxy(double x, double y);
extern char *fxy(double x, double y);
extern char *spt(GL_Point &pnt);
extern char *fpt(GL_Point &pnt);
static char *sdpt(GL_DPoint &pnt);
extern char *fdpt(GL_DPoint &pnt);
extern void InitPnts(int k);
