// $Header: /CAMCAD/4.6/UltimateGrid2005/UGCell.cpp 1     5/08/06 8:28p Kurt Van Ness $
...
{
   // Rick Faltersack - Bug fix 15 May 2009
   // When number cell has localization turned off this was still applying
   // localization, it had no flag checks. For German locale, this caused
   // periods to be removed (as thousands markers) hence turning all value to 
   // ints. So check of locale flag is being added.
   // Since this is purely for use in CAMCAD and since CAMCAD does not
   // support locale, the locale stuff probably should just be disabled
   // permanently, but I did not want to go that far.
   // This means one has to be sure to set the UGCELL_DONOT_LOCALIZE on cells
   // that might be processed through here.

	CString tempString;
	tempString = *string;

   if (!( m_propSetFlags&UGCELL_DONOT_LOCALIZE ))  //*rcf Added this
   {                                               //*rcf Added this
      //*rcf Original code block follows...
   ...
   //*rcf Since we are converting to number, I'll leave the trimming of currency marker in place
