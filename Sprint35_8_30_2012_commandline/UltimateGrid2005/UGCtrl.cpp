// $Header: /CAMCAD/4.6/UltimateGrid2005/UGCtrl.cpp 1     5/08/06 8:28p Kurt Van Ness $
...
****************************************************/
int CUGCtrl::EditCtrlFinished(LPCTSTR pString,BOOL cancelFlag,
							  BOOL continueFlag,int continueCol,
							  long continueRow)
{
   CString string(pString); // added 20071028 - knv
...
****************************************************/
BOOL CUGCtrl::AttachGrid(CWnd * wnd, UINT ID, bool headingFontBold){
	...
	if( SubclassDlgItem(ID,wnd))
	{
		...
		if(wnd!= NULL)
		{
         ...
         // RCF 19 Dec 2011 Added headingFontBold param to allow control of bold (or not) in
         // heading font. If bold is false then heading gets same font as data rows.
         if (headingFontBold)
         {
            nIndex = AddFont( logFont.lfHeight, logFont.lfWidth, logFont.lfEscapement,
               logFont.lfOrientation, logFont.lfWeight + 200, logFont.lfItalic,  // RCF The constant on this line
               logFont.lfUnderline, logFont.lfStrikeOut, logFont.lfCharSet,
               logFont.lfOutPrecision, logFont.lfClipPrecision, 
               logFont.lfQuality, logFont.lfPitchAndFamily, logFont.lfFaceName );
         }
...
#	else 
		...
		UGStr::tcscpy(tempStr,::strlen((LPCTSTR)*string), (LPCTSTR)*string);
#	endif
...
#	ifdef _UNICODE
	...
	UGStr::tcscpy(stringu, ::wcslen(*string), (LPCTSTR)*string);
...
// Added 20071025 - knv
void CUGCtrl::OnLClicked(int col,long row,int updn,UINT nFlags,RECT *rect,POINT *point,BOOL processed)
{
   OnLClicked(col,row,updn,rect,point,processed);
}
...
// Added 20071025 - knv
void CUGCtrl::OnRClicked(int col,long row,int updn,UINT nFlags,RECT *rect,POINT *point,BOOL processed)
{
...
// Added 20071025 - knv
void CUGCtrl::OnDClicked(int col,long row,UINT nFlags,RECT *rect,POINT *point,BOOL processed)
{
...
// Added 20071025 - knv
void CUGCtrl::OnTH_LClicked(int col,long row,int updn,UINT nFlags,RECT *rect,POINT *point,BOOL processed)
{
   OnTH_LClicked(col,row,updn,rect,point,processed);
}
...
// Added 20071025 - knv
void CUGCtrl::OnTH_RClicked(int col,long row,int updn,UINT nFlags,RECT *rect,POINT *point,BOOL processed)
{
   OnTH_RClicked(col,row,updn,rect,point,processed);
}
...
// Added 20071025 - knv
void CUGCtrl::OnTH_DClicked(int col,long row,UINT nFlags,RECT *rect,POINT *point,BOOL processed)
{
   OnTH_DClicked(col,row,rect,point,processed);
}
...
// Added 20071025 - knv
void CUGCtrl::OnSH_LClicked(int col,long row,int updn,UINT nFlags,RECT *rect,POINT *point,BOOL processed)
{
   OnSH_LClicked(col,row,updn,rect,point,processed);
}
...
// Added 20071025 - knv
void CUGCtrl::OnSH_RClicked(int col,long row,int updn,UINT nFlags,RECT *rect,POINT *point,BOOL processed)
{
   OnSH_RClicked(col,row,updn,rect,point,processed);
}
...
// Added 20071025 - knv
void CUGCtrl::OnSH_DClicked(int col,long row,UINT nFlags,RECT *rect,POINT *point,BOOL processed)
{
...
// added 20071028 - knv
int CUGCtrl::OnEditFinish(int col, long row,CWnd* edit,CString& string,BOOL cancelFlag)
{
   return OnEditFinish(col,row,edit,(const char*)string,cancelFlag);
}
