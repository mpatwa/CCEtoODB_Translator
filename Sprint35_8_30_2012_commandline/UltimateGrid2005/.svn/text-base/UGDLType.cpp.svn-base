...
, m_brush(NULL)
{
	...
	//m_pen.CreatePen(PS_SOLID,1,GetButtonColor());
	//m_brush.CreateSolidBrush(GetButtonColor());
...
COLORREF CUGDropListType::GetButtonColor()
{
...
CPen& CUGDropListType::getPen()
{
   if (m_pen == NULL)
   {
      m_pen = new CPen(PS_SOLID,1,GetButtonColor());
   }

   return *m_pen;
}

CBrush& CUGDropListType::getBrush()
{
   if (m_brush == NULL)
   {
      m_brush = new CBrush();
...
void CUGDropListType::OnDraw(CDC *dc,RECT *rect,int col,long row,CUGCell *cell,int selected,int current)
{
		...
		|| !UGXPThemes::DrawBackground(NULL, *dc, XPCellTypeCombo, UGXPThemes::GetState(selected > 0, current > 0), &rcCombo, NULL))
	{
		...
		//fill the border in
		dc->FillRect(&rectout,&getBrush());
		
		//make a line to separate the border from the rest ofthe cell
		oldpen = (CPen *)dc->SelectObject((CPen *)&getPen());
