...
STATIC DXF_ENTITY dx2_line_to_entity __(( DXF_FILE, C2_CURVE )) ;
STATIC DXF_ENTITY dx2_arc_to_entity __(( DXF_FILE, C2_CURVE )) ;
STATIC DXF_ENTITY dx2_pcurve_to_entity __(( DXF_FILE, 
            C2_PCURVE, REAL, REAL )) ;
STATIC DXF_ENTITY dx2_pc_buffer_to_entity __(( DXF_FILE, C2_PC_BUFFER, 
            DXF_ENTITY )) ;

/*-------------------------------------------------------------------------*/
BBS_PUBLIC DXF_ENTITY dx2_point_to_entity ( file, pt ) 
/*-------------------------------------------------------------------------*/
/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
/*!!!!!!!! (C) Copyright 1989 - 1995 Building Block Software   !!!!!!!*/
/*!!!!!!!!                    All rights reserved                   !!!!!!!*/
...
/*-------------------------------------------------------------------------*/
BBS_PUBLIC DXF_ENTITY dx2_curve_to_entity ( file, curve ) 
/*-------------------------------------------------------------------------*/
/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
/*!!!!!!!! (C) Copyright 1989 - 1995 Building Block Software   !!!!!!!*/
/*!!!!!!!!                    All rights reserved                   !!!!!!!*/
...
{ 
    if ( C2_CURVE_IS_LINE(curve) )
        RETURN ( dx2_line_to_entity ( file, curve ) ) ;
    else if ( C2_CURVE_IS_ARC(curve) )
        RETURN ( dx2_arc_to_entity ( file, curve ) ) ;
    else if ( C2_CURVE_IS_PCURVE(curve) ) 
        RETURN ( dx2_pcurve_to_entity ( file, C2_CURVE_PCURVE(curve), 
            C2_CURVE_T0(curve), C2_CURVE_T1(curve) ) ) ;
#ifdef SPLINE
    else if ( C2_CURVE_IS_ELLIPSE(curve) )
        RETURN ( dx2_ellnurb_to_entity ( file, C2_CURVE_NURB(curve), 
            C2_CURVE_PARM0(curve),  C2_CURVE_PARM1(curve) ) ) ;
    else if ( C2_CURVE_IS_SPLINE(curve) || C2_CURVE_IS_BEZIER(curve) )
        RETURN ( dx2_spline_to_entity ( file, C2_CURVE_NURB(curve), 
            C2_CURVE_PARM0(curve),  C2_CURVE_PARM1(curve) ) ) ;
#endif
    else
        RETURN ( NULL ) ;
...
/*-------------------------------------------------------------------------*/
STATIC DXF_ENTITY dx2_line_to_entity ( file, line ) 
/*-------------------------------------------------------------------------*/
/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
/*!!!!!!!! (C) Copyright 1989 - 1995 Building Block Software   !!!!!!!*/
/*!!!!!!!!                    All rights reserved                   !!!!!!!*/
...
/*-------------------------------------------------------------------------*/
STATIC DXF_ENTITY dx2_arc_to_entity ( file, curve ) 
/*-------------------------------------------------------------------------*/
/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
/*!!!!!!!! (C) Copyright 1989 - 1995 Building Block Software   !!!!!!!*/
/*!!!!!!!!                    All rights reserved                   !!!!!!!*/
...
{ 
    ...
    if ( fabs ( sweep ) <= TWO_PI - BBS_ZERO ) {
        ...
        if ( dir == 1 ) {
            ...
            if ( DXF_ARC_ANG0(entity) < 0.0 )
                DXF_ARC_ANG0(entity) += 360.0;
            DXF_ARC_ANG1(entity) = ang0 / PI_OVER_180 ;
            if ( DXF_ARC_ANG1(entity) < 0.0 )
                DXF_ARC_ANG1(entity) += 360.0;
        }
    }
    else {
...
/*-------------------------------------------------------------------------*/
STATIC DXF_ENTITY dx2_pcurve_to_entity ( file, pcurve, t0, t1 ) 
/*-------------------------------------------------------------------------*/
/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
/*!!!!!!!! (C) Copyright 1989 - 1995 Building Block Software   !!!!!!!*/
/*!!!!!!!!                    All rights reserved                   !!!!!!!*/
...
{ 
    ...
    if ( t0 > BBS_ZERO || t1 < (REAL) c2p_n(pcurve) - 1.0 - BBS_ZERO ) {
        ...
        pline = dx2_pcurve_to_entity ( file, tcurve, 
    ...
    if ( DXF_PLINE_VLIST(pline) == NULL )
        RETURN ( NULL ) ;
    dx2_pc_buffer_to_entity ( file, C2_PCURVE_BUFFER(pcurve), pline ) ;
    C2_WALK_PCURVE ( pcurve, item ) 
        dx2_pc_buffer_to_entity ( file, DML_RECORD(item), pline ) ;

    if ( !closed && c2p_ept1 ( pcurve, p ) ) {
        vertex = dx2_vertex_to_entity ( file, p, 0.0, 0 ) ;
        if ( vertex == NULL ) 
            RETURN ( NULL ) ;
...
/*-------------------------------------------------------------------------*/
STATIC DXF_ENTITY dx2_pc_buffer_to_entity ( file, buffer, pline ) 
/*-------------------------------------------------------------------------*/
/*!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!*/
/*!!!!!!!! (C) Copyright 1989 - 1995 Building Block Software   !!!!!!!*/
/*!!!!!!!!                    All rights reserved                   !!!!!!!*/
...
{ 
    ...
    for ( i=0 ; i<C2_PC_BUFFER_N(buffer)-1 ; i++ ) {
        ...
        vertex = dx2_vertex_to_entity ( file, C2_PC_BUFFER_PT(buffer,i), 
