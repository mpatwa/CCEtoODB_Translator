// $Header: /CAMCAD/4.5/Point3d.h 2     3/31/05 10:11a Kurt Van Ness $

#if !defined(__Point3d_H__)
...
// Comparison operators (used to define a sort order)
   ...
   bool operator!=(const CPoint3d& otherPoint) const {  return  x != otherPoint.x || y != otherPoint.y || z != otherPoint.z;  }
   //bool operator< (const CPoint3d& otherPoint) const;// {  return (x <  otherPoint.x) || (x == otherPoint.x && y <  otherPoint.y);  }
   //bool operator> (const CPoint3d& otherPoint) const;// {  return (x >  otherPoint.x) || (x == otherPoint.x && y >  otherPoint.y);  }
   //bool operator<=(const CPoint3d& otherPoint) const;// {  return (x <  otherPoint.x) || (x == otherPoint.x && y <= otherPoint.y);  }
   //bool operator>=(const CPoint3d& otherPoint) const;// {  return (x >  otherPoint.x) || (x == otherPoint.x && y >= otherPoint.y);  }
