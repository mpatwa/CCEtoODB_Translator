// $Header: /CAMCAD/4.3/TipDlg.cpp 8     8/12/03 7:25p Kurt Van Ness $
...
static CTipDlg *tipDlg = NULL;

void CCEtoODBApp::ShowTipAtStartup(void)
{
   CCommandLineInfo cmdInfo;
   ParseCommandLine(cmdInfo);
   if (cmdInfo.m_bShowSplash && !MostlyHideCamcad && !SilentRunning && ViewMode != SW_HIDE && ViewMode != SW_SHOWMINIMIZED &&
...
void CCEtoODBApp::ShowTipOfTheDay(void)
{
   if (tipDlg) // don't want 2 tip dialogs
      return;
   ...
   : CDialog(IDD_TIP, pParent)
{
   ...
   CString tipFile(getApp().getCamcadExeFolderPath());
   tipFile += "tips.txt";
   m_pStream = fopen(tipFile, "r");
   if (m_pStream == NULL) 
   {
