// $Header: /CAMCAD/4.3/ListControl.cpp 1     10/02/03 8:12a Kurt Van Ness $
...
#ifndef _winver
#define _winver WINVER
#endif
/*** Definition of class "CListControl" ***************************************/

IMPLEMENT_DYNCREATE(CListControl, CListCtrl)
...
LRESULT CListControl::OnFindItem(WPARAM wParam, LPARAM lParam)
{
  return CListBase::OnFindItem(wParam, lParam);
}

/*** The attributes of a list view control's column will be retrieved ********/
LRESULT CListControl::OnGetColumn(WPARAM wParam, LPARAM lParam)
{
...
/*** Draw small icon *********************************************************/
void CListBase::DrawSmallIcon(CDC* pDC, LVITEM* pItem, LPRECT pRect) const
{
  ASSERT(m_pListCtrl);
...
/*** Draw state icon *********************************************************/
void CListBase::DrawStateIcon(CDC* pDC, LVITEM* pItem, LPRECT pRect) const
{
  ASSERT(m_pListCtrl);
            ...
            -1)
        {
          ...
          if (!(m_pListCtrl->GetStyle() & LVS_OWNERDATA))
          {
            ...
            if (nPhysicalColumn == 0 && !bNoColumnWasVisible)
            {
              ...
              int nColumn1 = GetLogicalIndex(1);
              for (int nItem = m_pListCtrl->GetItemCount(); --nItem >= 0;)
              {
                LVITEM lvItem = {LVIF_PARAM, nItem, 0};
                      ...
                      LVM_GETITEM, 0, reinterpret_cast<LPARAM>(&lvItem)))
                {
                  LVITEM* pLVItem =
                    reinterpret_cast<ITEM_DATA*>(
                      lvItem.lParam)->m_apLVItem[nColumn1];

                  pLVItem->iItem    = nItem;
                  pLVItem->iSubItem = nColumn1;
                  m_pListCtrl->SetItem(pLVItem);
                }
              }
            }
...
LRESULT CListBase::OnDeleteAllItems()
{
       ...
       --nItem >= 0;)
  {
    ...
    if (m_pListCtrl->DefWindowProc(LVM_GETITEM, 0,
                                   reinterpret_cast<LPARAM>(&lvItem)) &&
        m_pListCtrl->DefWindowProc(LVM_DELETEITEM, nItem, 0))
...
BOOL CListBase::OnGetdispinfo(NMHDR* pNMHDR, LRESULT* pResult)
{
  ASSERT(m_pListCtrl);
